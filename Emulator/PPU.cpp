...
const int kBackgroundPatternTableAddressMask =
    1 << kBackgroundPatternTableAddressBit;
...
// PPU STATUS REGISTER
const int kPPUStatusSpriteOverflowBit =
    5;  // ~= More than 8 sprites on a scanline
...
bool PPU::render_scanline(int scanline) {
  ...
  if (scanline == 261) {
    ...
    // If VBlank interrupts are enabled, return true which will generate an
    // interrupt
...
dbyte PPU::calculate_effective_address(dbyte address) {
  ...
  if (address < 0x2000) {
    ...
    // There is only physical space for two name & attribute tables in the PPU
    // memory Mirroring bits in the cartridge control how the 4 virtual tables
    // are mapped
    //
...
/** VBLANK **/
void PPU::reset_vblank_flag() { status &= ~kPPUStatusVBlankMask; }

/** SPRITE HIT **/
void PPU::reset_sprite_0_flag() { status &= ~kPPUStatusSprite0Mask; }

void PPU::set_sprite_0_flag() { status |= kPPUStatusSprite0Mask; }
...
bool PPU::enable_background() { return control_2 & kBackgroundEnableMask; }

bool PPU::enable_sprites() { return control_2 & kSpritesEnableMask; }

bool PPU::use_8x16_sprites() { return control_2 & kSpriteSizeMask; }
...
byte PPU::read_control_1() { return control_1; }
void PPU::write_control_1(byte value) {
  ...
  regS = (value & kBackgroundPatternTableAddressMask) >>
         kBackgroundPatternTableAddressBit;
}

byte PPU::read_control_2() { return control_2; }
void PPU::write_control_2(byte value) { control_2 = value; }
void PPU::write_spr_ram(byte* start) { ::memcpy(spr_ram, start, kSprRAMSize); }
...
byte PPU::read_sprite_data() { return spr_ram[sprite_memory_address++]; }
...
dbyte PPU::vram_address() {
  return cntHT | cntVT << 5 | cntH << 10 | cntV << 11 | (cntFV & 0x03) << 12;
}

dbyte PPU::nametable_address() {
  return 0x2000 | cntHT | cntVT << 5 | cntH << 10 | cntV << 11;
}

// Use the attribute table to determine which palette to use. The byte in the
// name table determines the actual color to use in the palette. Returns the two
// high order bits of the palette entry.
...
dbyte PPU::attributetable_address() {
  return 0x23C0 | (cntHT & 0x1C) >> 2 | (cntVT & 0x1C) << 1 | cntH << 10 |
         cntV << 11;
}
...
dbyte PPU::patterntable_address() {
  ...
  return regS << 12 | pattern_index << 4 | cntFV;
}
...
bool PPU::is_screen_enabled() {
  return control_2 & kBackgroundEnableMask || control_2 & kSpritesEnableMask;
}
